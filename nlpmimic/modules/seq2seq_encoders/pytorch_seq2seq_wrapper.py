...
class MimicSeq2SeqWrapper(Seq2SeqEncoder):
    """
    ...
    def __init__(self, module: torch.nn.Module, stateful: bool = False) -> None:
        super(MimicSeq2SeqWrapper, self).__init__(stateful)
        self._module = module
        try:
    ...
    def forward(self,  # pylint: disable=arguments-differ
                inputs: torch.Tensor,
                mask: torch.Tensor,
                hidden_state: torch.Tensor = None) -> Tuple[torch.Tensor, Any]:
        ...
        final = final_states[0] # hidden states
        #print(final, final.size())
        xx = final[-2:]         # for- & back-ward states
        #print(xx, xx.size())
        yy = xx.index_select(1, restoration_indices)   
        #print(yy, yy.size())
        final = torch.cat([yy[0], yy[1]], -1) # final states of the forward and backward passes
        #print(final, final.size())
        #final = yy[0]           # final states of the forward pass
        #print(final, final.size())
        #import sys
        #sys.exit(0)

        # Restore the original indices and return the sequence.
        return unpacked_sequence_tensor.index_select(0, restoration_indices), final 
