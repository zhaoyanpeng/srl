...
@DatasetReader.register("penn_treebank")
class PennTreeBankConstituencySpanDatasetReader(DatasetReader):
    ...
    def text_to_instance(self, # type: ignore
        ...
        gold_spans = None # binary tree
        if gold_tree is not None:
            new_tree = gold_tree.copy(True)
            new_tree.chomsky_normal_form(horzMarkov=2)
            new_tree.collapse_unary(collapsePOS=False)
            gold_spans: Dict[Tuple[int, int], str] = {}
            self._get_gold_spans(new_tree, 0, gold_spans)
        
        metadata = {"tokens": tokens, "gold_spans": gold_spans}
        if gold_tree:
            metadata["gold_tree"] = new_tree
