...
@TrainerBase.register("srl_upper")
class VaeSrlTrainer(Trainer):
    ...
    def _create_batches(self, num_epochs: int = 1):
        ...
        if self.train_dx_data is not None and \
                self.train_dy_data is not None:
    ...
    def stupid(self):
        return self.iterator(self.train_data, 1, self.shuffle)
    ...
    def _train_epoch(self, epoch: int) -> Dict[str, float]:
        ...
        for batch in train_generator_tqdm:
            if batches_this_epoch % 50 == 0:
                peep = True 
                print('\n----------------------0. model.tau is {}'.format(self.model.tau))
            else:
