...
class GanSrlTrainer(Trainer):
    ...
    def _train_generator(self, 
                         ibatch: int,
                         fake: Iterator[TensorDict], 
                         real: Iterator[TensorDict] = None, 
                         peep: bool = False):
        self.optimizer.zero_grad()

        true = self.gen_pretraining >= 0

        if self.feature_matching: # caching features 
        ...
        if self.gen_pretraining > 0:       # semi-supervised 
            #logger.info('------semi-supervised')
            g_loss = gen_loss.item()
            gen_loss = gen_loss + ce_loss
            gen_loss.backward()
        ...
        return g_loss, ce_loss, kl_loss, bp_loss, gen_batch_grad_norm 
    ...
    def _train_epoch(self, epoch: int) -> Dict[str, float]:
        ...
        for noun_batch, batch_size in train_generator_tqdm:
            ...
            if epoch >= self.gen_skip_nepoch and optimize_gen:
                ...
                g_loss, ce_loss, kl_loss, bp_loss, gen_batch_grad_norm = self._train_generator(
