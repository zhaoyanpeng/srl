...
@TrainerBase.register("sri_vae_feats")
class VaeSrlTrainer(Trainer):
    ...
    def is_grad_nan(self, flag=""):
        print('{} grad begins'.format(flag))
        for n, p in self.model.named_parameters(): 
            if n in self.parameter_names and p.grad is not None:
                if torch.isnan(p.grad).any():
                    print('{} is nan'.format(n), p.grad.size())
        print('{} grad ends'.format(flag))

    def is_param_nan(self, flag=""):
        print('{} param begins'.format(flag))
        for n, p in self.model.named_parameters(): 
            if n in self.parameter_names and p.data is not None:
                if torch.isnan(p.data).any():
                    print('{} is nan'.format(n), p.data.size())
        print('{} param ends'.format(flag))
    ...
    def from_params(cls,   # type: ignore
        ...
        from nlpmimic.training.trainer import FeaturePieces
        pieces = FeaturePieces.from_params(params, serialization_dir, recover)  # pylint: disable=no-member
